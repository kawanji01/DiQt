...
class SentenceRequestSentenceListView extends StatefulWidget {
  const SentenceRequestSentenceListView(
      {super.key, required this.sentenceId, required this.selected});
  final int sentenceId;
  final String selected;

  @override
  State<SentenceRequestSentenceListView> createState() =>
      SentenceRequestSentenceListViewState();
}

class SentenceRequestSentenceListViewState
    extends State<SentenceRequestSentenceListView> {
  ...
  // pageのパラメーターの初期値
  final PagingController<int, SentenceRequest> _pagingController =
  ...
  Future<void> _fetchPage(int pageKey) async {
    ...
    final Map resMap = await RemoteSentenceRequests.listForSentence(
    ...
    final List<SentenceRequest> sentenceRequests = [];

    resMap['sentence_requests']
        .forEach((e) => sentenceRequests.add(SentenceRequest.fromJson(e)));

    final isLastPage = sentenceRequests.length < _pageSize;
    if (isLastPage) {
      _pagingController.appendLastPage(sentenceRequests);
    } else {
      _nextPagekey = pageKey + sentenceRequests.length;
      // _pagingController.appendLastPage(notices);
      // pageKeyにnullを渡すことで、addPageRequestListener の発火を防ぎ、自動で次のアイテムを読み込まないようにする。
      _pagingController.appendPage(sentenceRequests, _nextPagekey);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return PagedListView<int, SentenceRequest>(
      ...
      builderDelegate: PagedChildBuilderDelegate<SentenceRequest>(
        itemBuilder: (context, item, index) => SentenceRequestListItem(
          sentenceRequest: item,
        ),
