...
class QuizRequestQuizListView extends StatefulWidget {
  const QuizRequestQuizListView(
      {super.key, required this.quizId, required this.selected});
  final int quizId;
  final String selected;

  @override
  State<QuizRequestQuizListView> createState() =>
      QuizRequestQuizListViewState();
}

class QuizRequestQuizListViewState extends State<QuizRequestQuizListView> {
  ...
  // pageのパラメーターの初期値
  final PagingController<int, QuizRequest> _pagingController =
  ...
  Future<void> _fetchPage(int pageKey) async {
    ...
    final Map resMap = await RemoteQuizRequests.listForQuiz(
    ...
    resMap['quiz_requests']
        .forEach((e) => quizRequests.add(QuizRequest.fromJson(e)));

    final isLastPage = quizRequests.length < _pageSize;
    if (isLastPage) {
      _pagingController.appendLastPage(quizRequests);
    } else {
      _nextPagekey = pageKey + quizRequests.length;
      // _pagingController.appendLastPage(notices);
      // pageKeyにnullを渡すことで、addPageRequestListener の発火を防ぎ、自動で次のアイテムを読み込まないようにする。
      _pagingController.appendPage(quizRequests, _nextPagekey);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return PagedListView<int, QuizRequest>(
      ...
      builderDelegate: PagedChildBuilderDelegate<QuizRequest>(
        itemBuilder: (context, item, index) => QuizRequestListItem(
          quizRequest: item,
        ),
