...
import 'package:booqs_mobile/components/weakness/list_quiz.dart';
...
class WeaknessSolvedQuizListView extends ConsumerStatefulWidget {
  ...
  @override
  WeaknessSolvedQuizListViewState createState() =>
      WeaknessSolvedQuizListViewState();
}

class WeaknessSolvedQuizListViewState
  ...
  Future<void> _fetchPage(int pageKey) async {
    ...
    final Map? resMap =
        await RemoteWeaknesses.solved(pageKey, _pageSize, widget.order);
    if (!mounted) return;
    if (resMap == null) {
      return setState(() {
        _isLoading = false;
