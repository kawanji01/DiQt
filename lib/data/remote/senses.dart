...
class RemoteSenses {
  ...
  static Future<Map?> show(int sentenceId) async {
    try {
      final Uri url =
          Uri.parse('${DiQtURL.root()}/api/v1/mobile/senses/$sentenceId');
  ...
  static Future<Map?> newSense(int wordId) async {
    try {
      final Uri url = Uri.parse(
          '${DiQtURL.root()}/api/v1/mobile/senses/new?word_id=$wordId');
      final Response res = await HttpService.get(url);
      if (res.statusCode != 200) return null;
  ...
  static Future<Map?> edit(int senseId) async {
    try {
      final Uri url =
          Uri.parse('${DiQtURL.root()}/api/v1/mobile/senses/$senseId/edit');
      final Response res = await HttpService.get(url);
      if (res.statusCode != 200) return null;
  ...
  static Future<Map?> create(Map<String, dynamic> params) async {
    try {
      ...
      final Uri url = Uri.parse('${DiQtURL.root()}/api/v1/mobile/senses');
  ...
  static Future<Map?> update(Map<String, dynamic> params) async {
    try {
      ...
      final Uri url =
          Uri.parse('${DiQtURL.root()}/api/v1/mobile/senses/${params['id']}');
  ...
  static Future<Map?> destroy(int senseId) async {
    try {
      final Uri url =
          Uri.parse('${DiQtURL.root()}/api/v1/mobile/senses/$senseId');
